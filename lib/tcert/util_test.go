...
func TestValidateTCertBatchRequest(t *testing.T) {
	...
	var getBatch *idp.GetPrivateSignersGenKeyRequest
	...
	getBatch, batchError = getTemporalBatch(&batchRequest, 2)
	if batchError != nil {
...
func getTemporalBatch(batchRequest *idp.GetPrivateSignersRequest, count int) (*idp.GetPrivateSignersGenKeyRequest, error) {

	var priv *ecdsa.PrivateKey
	var err error
	var ecSignaure idp.ECSignature
	var signature idp.Signature
	var tempCrypto idp.TemporalCrypto

	batchRaw := fmt.Sprintf("%v", batchRequest)
	raw := []byte((batchRaw))

	var set []idp.TemporalCrypto
	for i := 0; i < count; i++ {
		...
		tempCrypto = idp.TemporalCrypto{PublicKey: pubASN1, Signature: signature}

		set = append(set, tempCrypto)

	}

	getBatch := idp.GetPrivateSignersGenKeyRequest{
		PrivateSignerRequest: batchRequest,
		SignatureBatch:       set,
	}
