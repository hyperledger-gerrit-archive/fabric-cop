...
func TestTCertWithoutAttribute(t *testing.T) {
	...
	resp, err := mgr.GetBatch(&GetBatchRequest{
		...
		AttrNames:                  nil,
		ValidityPeriod:             "",
		AttributeEncryptionEnabled: false,
		RootPreKey:                 "anyroot",
	}, ecert)
	if err != nil {
	...
	if len(resp.Certs) != 1 {
		t.Errorf("Returned incorrect number of certs: expecting 1 but found %d", len(resp.Certs))
	}
...
func TestTCertWitAttributes(t *testing.T) {
	...
	resp, err := mgr.GetBatch(&GetBatchRequest{
		...
		AttrNames:                  nil,
		ValidityPeriod:             "",
		AttributeEncryptionEnabled: true,
		AttributeSet:               AttributeSet,
		RootPreKey:                 "S5i15SgeDdd1pYVmaeA92B30Gq1cY8HHpoMHN5qpEu+ioK0gdUsJP2XI4wK43AQh",
	}, ecert)
	if err != nil {
		t.Errorf("Error from GetBatch: %s", err)
		return
	...
	if len(resp.Certs) != 2 {
		t.Errorf("Returned incorrect number of certs: expecting 2 but found %d", len(resp.Certs))
	}
...
func getMgr(validityPeriod string, t *testing.T) *Mgr {
	...
	template, err := LoadCert("../../testdata/test.pem")
	if err != nil {
		t.Errorf("failed loading tcert template: %s", err)
		return nil
	}
	mgr, err := NewMgr(template, caCert, caKey, validityPeriod)
	if err != nil {
