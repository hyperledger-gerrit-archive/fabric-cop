...
func GetAttributeManagerInstance(certBuf []byte, keys map[string][]byte) (*AttributeMgr, error) {
	...
	if error != nil {
		errorMessage := fmt.Sprintf("Retrieving attributes from tcert failed with error : %s", error)
...
func processAttributeHeader(headerValue string, extension []pkix.Extension, keys map[string][]byte) ([]Attribute, error) {
	...
	for i := 0; i < attributeBlockLength; i++ {
		if attributeBlock[i] != "" {
			...
			if err != nil {
				errorMessage = fmt.Sprintf("String to Integer conversion for attribute[%s] failed with error : %s", err, attributeName)
				log.Error(errorMessage)
				return nil, errors.New(errorMessage)
			}
			...
			if shouldDecrypt {
				...
				if err != nil {
					errorMessage = fmt.Sprintf("The attribute value decryption for [%s] failed with error : %s", attributeName, err)
					log.Error(errorMessage)
					return nil, errors.New(errorMessage)
				}
