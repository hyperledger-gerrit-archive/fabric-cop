...
//             False otherwise
func (tm *Mgr) VerifyTcertBatchRequest(tcertbatch *idp.GetPrivateSignersGenKeyRequest) (bool, error) {

	var errorMsg string
	if tcertbatch == nil {
		return false, errors.New("Request is nil. Please provide valid request")
	}
	batchRequest := tcertbatch.PrivateSignerRequest
	if &batchRequest == nil {
		return false, errors.New("Request is nil. Please provide valid request")
	}
	...
	tcertBatchRequestRaw := fmt.Sprintf("%v", batchRequest)
	rawTcerBatch := []byte((tcertBatchRequestRaw))
	...
	for i := 0; i < noOfTemporalSignatures; i++ {
		...
		isValid, validationError := tm.VerifyMessage(rawTcerBatch, publicKey, signature)
