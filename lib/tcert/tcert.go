...
type Mgr struct {
	...
	// Template is used as the basis from which to create all tcerts
	// The default template is the CA's certificate
	Template *x509.Certificate
	// MaxBatchSize is the maximum number of tcerts which can be requested at a time
...
func (tm *Mgr) GetBatch(req *GetBatchRequest, ecert *x509.Certificate) (*GetBatchResponse, error) {
	...
	// Enforce the max tcert batch size, and if 0 are requested, change to 1
	var num int
	if req.Count <= 0 {
		num = 1
	} else if req.Count > tm.MaxBatchSize {
		num = int(tm.MaxBatchSize)
	} else {
		num = int(req.Count)
	}
...
func generateExtensions(tcertid *big.Int, tidx []byte, enrollmentCert *x509.Certificate, batchRequest *GetBatchRequest) ([]pkix.Extension, map[string][]byte, error) {
	...
	for i := 0; i < len(attrs); i++ {
		...
		if batchRequest.AttrEncryption {
			mac = hmac.New(sha512.New384, preK0)
			mac.Write([]byte(name))
