...
func (tm *Mgr) GetBatch(req *GetBatchRequest, ecert *x509.Certificate) (*GetBatchResponse, error) {
	...
	for i := 0; i < numTCertsInBatch; i++ {
		...
		if certError != nil {
			...
			log.Errorf("Failed to generate a Transaction Certificate (TCert) [%v]", certError)
...
func (tm *Mgr) GetBatchForGeneratedKey(req *GetBatchForKeysRequest) (*GetBatchResponse, error) {
	var errorMessage string
	if req == nil {
		errorMessage = "Batch Request is nil"
		log.Error(errorMessage)
