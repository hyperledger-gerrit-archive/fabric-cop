...
//returns batch of tcerts in JSON format
func GetCertificateSet(jsonString string, signatureString string) (string, error) {
	...
	tcertBatchRequest := &BatchRequest{}
	requestJSONError := json.Unmarshal([]byte(jsonString), tcertBatchRequest)
	if requestJSONError != nil {
		return "", errors.New("Input json could not be mashalled")
	}

	certificate, certificateError := crypto.GetCertFromSignatureJSON(signatureString)
	if certificateError != nil {
		log.Error(certificateError)
		return "", certificateError
	}
	tcertResponse, tcertResponseError := GenerateTCertBatch(tcertBatchRequest, certificate)

	if tcertResponseError != nil {
		log.Error(tcertResponseError)
		return "", tcertResponseError
	}

	return tcertResponse, nil
}

//GenerateTCertBatch returns
func GenerateTCertBatch(batchRequest *BatchRequest, cert *x509.Certificate) (string, error) {
	...
	for i := 0; i < num; i++ {
		...
		extensions, ks, extensionErr := generateExtensions(tcertid, encryptedTidx, cert, batchRequest)
		...
		spec, specError := crypto.ParseCertificateRequest(&batchRequest.CertificateRequestData, uid, tcertid, &txPub, x509.KeyUsageDigitalSignature, extensions)

		if specError != nil {
			return "", specError
		}

		certbuff, certbufferr := crypto.NewCertificateFromSpec(spec)
		if certbufferr != nil {
			log.Errorf("Error in creating tcer = [%v]", certbufferr)
			return "", certbufferr
		}

		set = append(set, TCert{certbuff, ks})
	}

	//Id is hardcoded right now. Need to look into Timestamp part as well. Removed timestamp for the time being
	tcertID := crypto.GenNumber(big.NewInt(20))
	tcertResponse := CreateSetResp{&CertSet{tcertID, time.Now(), kdfKey, set}}
	//return &tcertResponse, nil
	...
	tcertResponseString := string(tcertResponseJSONByte)
	return tcertResponseString, nil
}

// Generate encrypted extensions to be included into the TCert (TCertIndex, EnrollmentID and attributes).
func generateExtensions(tcertid *big.Int, tidx []byte, enrollmentCert *x509.Certificate, batchRequest *BatchRequest) ([]pkix.Extension, map[string][]byte, error) {
	...
	enrollmentID := []byte(crypto.GetEnrollmentIDFromCert(enrollmentCert))
	...
	for i := 0; i < len(attributeSet); i++ {
		count++
		AttributeName = attributeSet[i].AttributeName
		AttributeValue = attributeSet[i].AttributeValue
		...
		if batchRequest.AttributeEncryptionEbabled {
			mac = hmac.New(sha512.New384, preK0)
