...
func TestRSAPKCSPrivateKey(t *testing.T) {
	pkcs8privKeyBuff, pkerr := ioutil.ReadFile("../../testdata/pkcs8rsa-key.pem")
	if pkerr != nil {
		t.Fatalf("Cannot read PKCS8 Private Key from file system")
	}
	_, pkcs8err := GetPrivateKey(pkcs8privKeyBuff)
	if pkcs8err != nil {
		t.Fatalf("Cannot parse PKCS8 Private Key from PEM")
	}
}

func TestECPKCSPrivateKey(t *testing.T) {
	pkcs8privKeyBuff, pkerr := ioutil.ReadFile("../../testdata/pkcs8ec-key.pem")
	if pkerr != nil {
		t.Fatalf("Cannot read PKCS8 Private Key from file system")
	}
	_, pkcs8err := GetPrivateKey(pkcs8privKeyBuff)
...
func TestCertFromSignatureJSON(t *testing.T) {

	privateKeyBuff, err := ioutil.ReadFile("../../testdata/ecTest-key.pem")
	if err != nil {
		t.Fatalf("TestCertFromSignatureJSON : Cannot read EC Private Key File")
	}
	privateKey, err := GetPrivateKey(privateKeyBuff)
	if err != nil {
		t.Fatalf("TestCertFromSignatureJSON : Cannot Generate EC Private Key Object")
	}

	CertificateBuff, err := ioutil.ReadFile("../../testdata/ecTest.pem")
	if err != nil {
		t.Fatalf("TestCertFromSignatureJSON : Cannot read EC Certificate File")
	}
	...
	cert, certError := GetCertFromSignatureJSON(signedJSON)
	if certError != nil || cert == nil {
		t.Fatalf("TestCertFromSignatureJSON : Certificate object cannot be obtained")
	}
}
