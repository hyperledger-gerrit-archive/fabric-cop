...
func GetEnrollmentIDFromCert(enrollmentCert *x509.Certificate) string {
	enrollmentID := enrollmentCert.Subject.CommonName
	return enrollmentID
}
...
//GetCertFromSignatureJSON  gets Certificate Object from signature json
func GetCertFromSignatureJSON(signatureString string) (*x509.Certificate, error) {

	signature := &Signature{}
	signatureJSONError := json.Unmarshal([]byte(signatureString), signature)
	if signatureJSONError != nil {
		log.Error(signatureJSONError)
		return nil, signatureJSONError
	}

	certificateByte, decodingError := base64.StdEncoding.DecodeString(signature.Certificate)
	if decodingError != nil {
		log.Error(decodingError)
		return nil, errors.New("ECert Could not be base64 decoded")
	}

	certificate, certError := GetCertificate(certificateByte)
	if certError != nil {
		log.Error(certError)
