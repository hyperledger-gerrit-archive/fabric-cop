...
func configInit(cfg *cli.Config) {
	...
	CFG.CAFile = abs2cfg(CFG.CAFile)
	CFG.KeyFile = abs2cfg(CFG.KeyFile)
	CFG.TLSConf.TLSCertFile = abs2cfg(CFG.TLSConf.TLSCertFile)
	CFG.TLSConf.TLSKeyFile = abs2cfg(CFG.TLSConf.TLSKeyFile)
	CFG.TLSConf.MutualTLSCAFile = abs2cfg(CFG.TLSConf.MutualTLSCAFile)
	...
	if CFG.DBdriver == sqlite {
		CFG.DataSource = abs2home(CFG.DataSource)
	}
...
func absTLSClient(cfg *tls.ClientTLSConfig) {
	for i := 0; i < len(cfg.CACertFiles); i++ {
		cfg.CACertFiles[i] = abs2cfg(cfg.CACertFiles[i])
	}
	cfg.Client.CertFile = abs2cfg(cfg.Client.CertFile)
	cfg.Client.KeyFile = abs2cfg(cfg.Client.KeyFile)
}

// Make 'file' absolute relative to the configuration directory
func abs2cfg(file string) string {
	return abs(file, configDir)
}

// Make 'file' absolute relative to the home directory
func abs2home(file string) string {
	return abs(file, homeDir)
}

func abs(file, dir string) string {
