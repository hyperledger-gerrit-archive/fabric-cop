...
func createSQLiteDBTables(datasource string) error {
	...
	if _, err := db.Exec("CREATE TABLE IF NOT EXISTS groups (name VARCHAR(64), parent_id VARCHAR(64), key VARCHAR(48))"); err != nil {
		return err
	}
...
func createPostgresDBTables(datasource string, dbName string, db *sqlx.DB) error {
	...
	if _, err := database.Exec("CREATE TABLE groups (name VARCHAR(64), parent_id VARCHAR(64), key VARCHAR(48))"); err != nil {
		return err
	}
