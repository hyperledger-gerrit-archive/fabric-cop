...
func (d *Accessor) LoginUserBasicAuth(user, pass string) (spi.User, error) {
	...
	if err != nil {
		...
		return nil, cop.NewError(cop.AuthorizationError, "User (%s) not registered [error: %s]", user, err)
	}
	...
	if userRec.Pass == pass {
		if userRec.State == 0 {
			...
			res, err := d.db.Exec("UPDATE users SET state = ? WHERE (id = ?)", "1", user)
			if err != nil {
				return nil, err
			}
		...
		return nil, cop.NewError(cop.AuthorizationError, "User has already been enrolled")
	}

	log.Errorf("Incorrect username/password provided")
	return nil, cop.NewError(cop.AuthorizationError, "Incorrect username/password provided)")
}
...
func (d *Accessor) InsertUser(user spi.UserInfo) error {
	...
	log.Debugf("User (%s) inserted into database successfully\n", user.Name)
	return nil
